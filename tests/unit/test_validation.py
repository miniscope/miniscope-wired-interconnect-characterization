...
class TestTypeFieldValidator:
    @pytest.fixture
    def full_definition(self, full_definition_path: Path) -> MeasurementDefinition:
        with open(full_definition_path) as f:
            raw = yaml.safe_load(f)
        return MeasurementDefinition.model_validate(raw)

    def test_valid_fields(self, full_definition: MeasurementDefinition):
    ...
    def test_invalid_enum_value(self, full_definition: MeasurementDefinition):
        validator = TypeFieldValidator(full_definition)
        errors = validator.validate(
            {
                "cable_model": "x",
    ...
    def test_wrong_type(self, full_definition: MeasurementDefinition):
        validator = TypeFieldValidator(full_definition)
        errors = validator.validate(
            {
                "cable_model": "x",
    ...
    def test_optional_field_not_required(self, full_definition: MeasurementDefinition):
    ...
    def test_int_accepted_for_float(self, full_definition: MeasurementDefinition):
