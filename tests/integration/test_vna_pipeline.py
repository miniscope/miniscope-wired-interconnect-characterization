...
class TestVNAPipelineIntegration:
    ...
    def test_process_example_experiment(self, tmp_path: Path):
        repo_root = Path(".")
        example_dir = repo_root / "experiments" / "EXP_2025_03_01_vna_coax_40awg"
        ...
        exp_dir = test_repo / "experiments" / "EXP_2025_03_01_vna_coax_40awg"
        ...
        assert len(df) == 3  # 3 .s2p files

        traces = pd.read_csv(result.outputs["vna_traces_csv"])
        assert len(traces) == 3 * 201  # 3 files * 201 points
    ...
    def test_aggregate_after_processing(self, tmp_path: Path):
        ...
        outputs = aggregate_type("vna_characterization", test_repo)
        ...
        assert len(df) == 1
        assert df.iloc[0]["experiment_id"] == "EXP_2025_03_01_vna_coax_40awg"
