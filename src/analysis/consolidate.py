...
- mass_by_length.csv      (one row per length)
...
def _consolidate_mass(entries: list[tuple[SessionRecord, dict]]) -> list[dict]:
    ...
    for condition in _sorted_conditions(by_condition):
        ...
        summaries = [s for _, s in sessions_summaries]
        length_mm = sessions_summaries[0][0].cable_length_mm
        per_cm = _mean_std_n([s.get("mean_cable_mass_g_per_cm") for s in summaries])
        absolute = _mean_std_n([s.get("mean_cable_mass_g") for s in summaries])
        rows.append(
            {
                "condition": condition,
                "cable_length_mm": length_mm,
                "n_sessions": absolute["n_sessions"],
                "total_measurements": sum(s.get("num_measurements", 0) for s in summaries),
                "mean_cable_mass_g": absolute["mean"],
                "std_cable_mass_g": absolute["std"],
                "mean_cable_mass_g_per_cm": per_cm["mean"],
                "std_cable_mass_g_per_cm": per_cm["std"],
            }
        )
    return rows
...
def consolidate_profile(repo_root: Path, profile_id: str) -> dict[str, Path]:
    ...
    consolidators = {
        ...
        "mass": ("mass_by_length", _consolidate_mass),
