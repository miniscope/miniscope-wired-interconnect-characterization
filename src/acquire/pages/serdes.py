...
def serdes_page(profile_id: str, condition: str) -> None:
    ...
    async def repeat_margins() -> None:
        ...
        try:
            ...
            with shared["lock"]:
                shared["events"].clear()
            # A margin sweep reaching its floor by dropping lock is the measured
            # floor, not a failure (same as the full capture), so the new sweeps
            # are always pooled in. sweep_margins_only already skips lanes that no
            # longer link, so a no-link lane simply contributes nothing.
            existing.margins.extend(new_margins)
            render_final_results(existing)
            render_margin_summary(margin_summary, existing)
            floor_note = (
                " (some lanes hit a lock-loss floor)"
                if any(p.status in LOCK_LOSS_STATUSES for s in new_margins for p in s.points)
                else ""
            )
            status_label.text = (
                f"Added {runs} more margin iteration(s) per lane{floor_note} -- "
