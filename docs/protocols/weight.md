...
when budgeting tether weight on a moving animal.
...
1. **Weigh the fixture.** Put the bare PCB(s) + SMA connector(s) -- the
   ...
   the mass in grams as `fixture_weight_g`.
...
- `assembly_weight_g` must exceed `fixture_weight_g` -- a non-positive net
